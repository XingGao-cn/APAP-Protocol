...
function [node,alive1,die_node,All]=PRUncertain_stage(node,id,data_id,Time,M,realiability,All,alive1)
...
%alive1=1;
die_node=0;
[M,TTL,All] = Proliferation_and_Note(node,id,realiability,M,All);
       ...
       if id==data_id %源节点感知数据
        for i=1:M
           ...
           %*********将复制产生的数据包存放在本地**********
                   for i=1:M
                        node(id).Data(L(i)).ID=data_id;
                        node(id).Data(L(i)).start_time=Time+PR_dalay;
                        node(id).Data(L(i)).num=i;
                        node(id).Data(L(i)).PRs=M;
