...
function [node,alive1,die_node,intercepts,All] = per_hop(node,id,time,intercepts,All,alive1)
...
%        if node(receive_id).pass(insert_pos1).copies >1   
%          [node]=inform_table_member(node,receive_id,data_id);
%        end
%        end
%        end
       %***********************
...
%        end
       ...
       %***********************
            ...
            [node,alive1,die_node,All]=PRUncertain_stage(node,id,node(id).Data(i).ID,time,M,0.85,All,alive1);
             %清空本地记录
                node(id).Data(i).ID=0;
                node(id).Data(i).start_time=0;
                node(id).Data(i).num=0;
