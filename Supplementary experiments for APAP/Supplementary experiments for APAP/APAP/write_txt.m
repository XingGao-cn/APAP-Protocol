...
A=[a',b',c'];
fid=fopen('..\Data statistics code(part)\三维图数据集.txt','wt');%改为你自己文件的位置  %  这个是我的方法 file address
...
%  z=C(:,1);
% 
% figure,scatter3(x,y,z)%散点图 Scatter plot
%**************************************
...
cc=gradient(zz);
figure
meshz(xx,yy,zz,cc);shading interp%伪彩色图 Pseudo color map
...
figure
surfc(xx,yy,zz,cc);shading interp%伪彩色图
zlabel('Energy Consumption(J)')
...
% %插值 interpolation points
% figure
% pcolor(xx,yy,zz);shading interp%伪彩色图
% 
% %figure
% figure,contourf(xx,yy,zz) %等高线图 Contour map
% %figure
% figure,surf(xx,yy,zz);%三维曲面 3D surface
