%testing the key parameters D&I&U used in active packet-loss for one round
%main
clear,clc;
...
intercept3=0;
LL=20;%表的长度
Update0 = 1;%记录刷新时间，等价于Update_table中的interval1
[node]=construct_table(node,1,10,LL);
[node]=construct_table(node,2,20,LL);
...
for i=1:N/L
    ...
    for j=1:length(queue)
       ...
       [node,alive1,die_node,All,messages]=PRUncertain_stage(node,queue(j),queue(j),Time,3,0.85,All,alive1,messages);  
        ...
        [node]=Update_table(node,circle_1,Update0);
        [node]=Update_table(node,circle_2,Update0);
        [node]=Update_table(node,circle_3,Update0);
end
...
%Clear the global traffic load container 
for i=1:3500
...
for i=1:length(node)-1
 ...
 turn %Print the prompt message
 
if turn~=0
    Intercepts(turn)=length(intercepts);
end

e2e(50)=0;%E2E reliability
...
% avg=0;
% for i=1:length(node)
%    if node(i).ring<=3 && node(i).ring>0
%         nei=nei+1;
%         sum=sum+length(node(i).table_member)
%    end
% end
% avg=sum/nei
% length(node(901).receive)